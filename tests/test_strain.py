...
def SKA_WN():
    SKA_WN = detector.PTA('SKA, WN Only',
        N_p_SKA,T_obs=T_SKA,sigma=sigma_SKA,cadence=cadence_SKA,f_low=1e-10,f_high=1e-7,nfreqs=int(1e3))
    return SKA_WN

#SKA with White and Varied Red Noise
def test_pta_SKA_WN_RN():
    SKA_WN_RN = detector.PTA('SKA, WN and RN',
        N_p_SKA,T_obs=T_SKA,sigma=sigma_SKA,cadence=cadence_SKA,
...
def SKA_Sampled_Noise():
    SKA_Sampled_Noise = detector.PTA('SKA, Sampled Noise',
        N_p_SKA,cadence=[cadence_SKA,cadence_SKA/4.],
        sigma=[sigma_SKA,10*sigma_SKA],T_obs=T_SKA,
        use_11yr=True,use_rn=True)
    SKA_Sampled_Noise.h_n_f
    return SKA_Sampled_Noise

def test_pta_SKA_Sampled_Noise_GWB(SKA_Sampled_Noise):
    SKA_Sampled_Noise_GWB = detector.PTA('SKA, Sampled Noise and GWB',
        N_p_SKA,cadence=SKA_Sampled_Noise.cadence,
        sigma=SKA_Sampled_Noise.sigma,T_obs=SKA_Sampled_Noise.T_obs,
        rn_amp=SKA_Sampled_Noise.rn_amp,rn_alpha=SKA_Sampled_Noise.rn_alpha,
        phi=SKA_Sampled_Noise.phi,theta=SKA_Sampled_Noise.theta,
        sb_amp=4e-16)
...
def test_pta_NANOGrav_Sampled_Noise_GWB():
    NANOGrav_Sampled_Noise_GWB = detector.PTA('NANOGrav, Sampled Noise and GWB',
        N_p_nano,use_11yr=True,use_rn=True,sb_amp=4e-16,nbins=10)
...
def test_BBHStrain(LISA_prop1,aLIGO,SKA_WN,ET):
    ...
    source_3.Check_Freq_Evol(T_evol=np.max(source_3.instrument.T_obs).to('s'),T_evol_frame='source')
