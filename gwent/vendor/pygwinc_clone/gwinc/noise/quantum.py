...
def shotrad(f, ifo):
    ...
    Nfield = Msig.shape[0]
    Nfreq = len(f)
    if any(np.array([Mifo.shape[0], Mifo.shape[1], Mn.shape[0]]) != Nfield) or any(
        np.array([Mifo.shape[2], Msig.shape[2], Mn.shape[2]]) != Nfreq
    ):
    ...
    if ifoRead is None:
        ...
        eta = np.sign(ifoRead.fringe_side) * np.arccos(
            (ifoRead.defect_PWR_W / ifoRead.readout_PWR_W) ** 0.5
        )
    ...
    if sqzType == "None":
        ...
        lambda_in = (
            ifo.Squeezer.InjectionLoss
        )  # Loss to squeezing before injection [Power]
    ...
    if sqzType == "None":
        ...
        logging.debug(
            "You are injecting %g dB of frequency independent squeezing" % SQZ_DB
        )
        ...
        logging.debug(
            "You are injecting %g dB of squeezing with optimal frequency dependent squeezing angle"
            % SQZ_DB
        )
        ...
        logging.debug(
            "You are injecting %g dB of squeezing with optimal FD squeezing angle, for optimal readout phase"
            % SQZ_DB
        )

    elif sqzType == "Freq Dependent":
        logging.debug(
            "You are injecting %g dB of squeezing with frequency dependent squeezing angle"
            % SQZ_DB
        )

    else:
        raise Exception(
            'ifo.Squeezer.Type must be None, Freq Independent, Optimal, or Frequency Dependent, not "%s"'
            % sqzType
        )
    ...
    if sqzType == "Freq Dependent" and "FilterCavity" in ifo.Squeezer:
        logging.debug(
            "  Applying %d input filter cavities"
            % np.atleast_1d(ifo.Squeezer.FilterCavity).size
        )
    ...
    if "OutputFilter" in ifo:
        if ifo.OutputFilter.Type == "None":
            ...
            logging.debug(
                "  Applying %d output filter cavities"
                % np.atleast_1d(ifo.OutputFilter.FilterCavity).size
            )

            Mr, Mnoise = sqzFilterCavityChain(
                f, np.atleast_1d(ifo.OutputFilter.FilterCavity), Mnoise
            )
            ...
            raise Exception(
                'ifo.OutputFilter.Type must be None, Chain or Optimal, not "%s"'
                % ifo.OutputFilter.Type
            )
    ...
    def HDnoise(eta):
        ...
        n = (
            coeff
            * np.squeeze(sum(abs(getProdTF(vHD, Mnoise)) ** 2, axis=1))
...
def compile_ARM_RES_TF():
    ...
    print(
        ...
        "=",
        str(ARM_RES_expr[0])
        .replace("Matrix", "np.array")
        .replace(", 0]", ", np.zeros(nf)]"),
    )
...
def compile_SEC_RES_TF():
    ...
    (
        ...
        "phi exp_1jOmegal_c tArm exp_2jOmegaL_c K r00 r10 r11 R T rITM tSR rho"
    )
...
def shotradSignalRecycled(f, ifo):
    ...
    SEr = np.array(
        [
            [np.tile(cos(phi), nf), np.tile(sin(phi), nf)],
    ...
    ARM = (
        tArm
        * exp_2jOmegaL_c
        * np.array([[np.ones(nf), np.zeros(nf)], [-K, np.ones(nf)]])
    )
...
def shotradSignalRecycledBnC(f, ifo):
    ...
    # - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
    D1_L = (
        -(1 + rho * exp_2jbeta) * sin(phi)
        + 1
        / 4
        * epsilon
        * (3 + rho + 2 * rho * exp_4jbeta + exp_2jbeta * (1 + 5 * rho))
        * sin(phi)
        + 1 / 2 * lambda_SR * exp_2jbeta * rho * sin(phi)
    )

    D2_L = (
        -(-1 + rho * exp_2jbeta) * cos(phi)
        + 1
        / 4
        * epsilon
        * (-3 + rho + 2 * rho * exp_4jbeta + exp_2jbeta * (-1 + 5 * rho))
    ...
    N11 = (
        ...
        * tau
        * (
            Kappa * (1 + rho * exp_2jbeta) * sin(phi)
            + 2
            * cos_beta
            * (
                invexp_1jbeta * cos(phi)
                - rho * exp_1jbeta * (cos(phi) + Kappa * sin(phi))
            )
        )
    )
    N22 = (
