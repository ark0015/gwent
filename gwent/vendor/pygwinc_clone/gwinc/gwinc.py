...
def gwinc(freq, ifoin, source=None, plot=False, PRfixed=True):
    ...
    if plot:
        ...
        if ifo.Optics.Type == "DualCarrier_new":  # include the case for Dual carrier
            ...
            logging.info(
                "Arm power for carrier A:%7.2f kW"
                % (finesseA * 2 / pi * pbsA / 2 / 1000)
            )
            logging.info(
                "Arm power for carrier B:%7.2f kW"
                % (finesseB * 2 / pi * pbsB / 2 / 1000)
            )
            ...
            logging.info(
                "SRM Detuning for Carrier A:    %7.2f degree"
                % (ifo.Optics.SRM.TunephaseD1 * 180 / pi)
            )
            logging.info(
                "SRM Detuning for Carrier B:    %7.2f degree"
                % (ifo.Optics.SRM.TunephaseD2 * 180 / pi)
            )
            logging.info(
                "SRM transmission for Carrier A:%9.4f" % ifo.Optics.SRM.TransmittanceD1
            )
            logging.info(
                "SRM transmission for Carrier B:%9.4f" % ifo.Optics.SRM.TransmittanceD2
            )
            logging.info(
                "ITM transmission for Carrier A:%9.4f" % ifo.Optics.ITM.TransmittanceD1
            )
            logging.info(
                "ITM transmission for Carrier B:%9.4f" % ifo.Optics.ITM.TransmittanceD2
            )
            logging.info(
                "PRM transmission for both:     %9.4f" % ifo.Optics.PRM.Transmittance
            )
        else:
            logging.info("Laser Power:            %7.2f Watt" % ifo.Laser.Power)
            logging.info(
                "SRM Detuning:           %7.2f degree"
        ...
        PowAbsITM = (pbs / 2) * np.hstack(
            [
                (finesse * 2 / pi) * ifo.Optics.ITM.CoatingAbsorption,
        ...
        if source:
            logging.info(
                ...
                + str(score.effr0ns)
                + " Mpc/ z = "
                + str(score.zHorizonNS)
            )
            logging.info(
                "BBH Inspiral Range:     "
                + str(score.effr0bh)
                + " Mpc/ z = "
                + str(score.zHorizonBH)
            )
