...
def Plot_SNR(
    ...
    if yaxis_type == "lin" and xaxis_type == "log":
        CS1 = ax1.contourf(
            np.log10(sample_x), sample_y, logSNR, logLevels, cmap=colormap
        )
        ax1.contour(
            np.log10(sample_x), sample_y, logSNR, print_logLevels, colors="k", alpha=1.0
        )
        ...
        x_labels = np.logspace(
            np.log10(xlabel_min),
            np.log10(xlabel_max),
            int(np.log10(xlabel_max) - np.log10(xlabel_min) + 1),
        )
        y_labels = np.linspace(
            ylabel_min, ylabel_max, min(20, max(int(ylabel_max - ylabel_min + 1), 10))
        )
        ax1.set_yticks(y_labels)
        ax1.set_xticks(np.log10(x_labels))
    elif yaxis_type == "log" and xaxis_type == "lin":
        CS1 = ax1.contourf(
            sample_x, np.log10(sample_y), logSNR, logLevels, cmap=colormap
        )
        ax1.contour(
            sample_x, np.log10(sample_y), logSNR, print_logLevels, colors="k", alpha=1.0
        )
        ax1.set_xlim(xlabel_min, xlabel_max)
        ax1.set_ylim(np.log10(ylabel_min), np.log10(ylabel_max))
        x_labels = np.linspace(
            xlabel_min, xlabel_max, min(20, max(int(xlabel_max - xlabel_min + 1), 10))
        )
        y_labels = np.logspace(
            np.log10(ylabel_min),
            np.log10(ylabel_max),
            int(np.log10(ylabel_max) - np.log10(ylabel_min) + 1),
        )
        ...
        CS1 = ax1.contourf(
            np.log10(sample_x), np.log10(sample_y), logSNR, logLevels, cmap=colormap
        )
        ax1.contour(
            np.log10(sample_x),
            np.log10(sample_y),
    ...
    if var_x == "M":
        ...
        ax1.set_xticklabels(
            [x if int(x) < 1 else int(x) for x in x_labels], fontsize=axissize
        )
        ...
        ax1.axvline(
            x=np.log10(2.5 * u.Gm.to("m")),
            linestyle="--",
            color="k",
            label="Proposed arm length",
        )
        ...
        ax1.axvline(
            ...
            linestyle="--",
            color="k",
            label="Proposed Acceleration Noise Amplitude",
        )
        ax1.set_xlabel(
            r"$\mathrm{Acceleration~Noise~Amplitude}$ $[\mathrm{m~s^{-2}}]$",
            fontsize=labelsize,
        )
        ax1.set_xticklabels(
            [r"$10^{%.0f}$" % x for x in np.log10(x_labels)], fontsize=axissize
        )
        ...
        ax1.axvline(
            ...
            linestyle="--",
            color="k",
            label="Proposed Optical Metrology Noise Amplitude",
        )
        ax1.set_xlabel(
            r"$\mathrm{Optical~Metrology~Noise~Amplitude}~[\mathrm{m}]$",
            fontsize=labelsize,
        )
        ax1.set_xticklabels(
            [r"$10^{%.0f}$" % x for x in np.log10(x_labels)], fontsize=axissize
        ...
        x_labels = (
            np.arange(round(xlabel_min / scale), round(xlabel_max / scale) + 1, 1)
            * scale
        )
        ax1.set_xticks(x_labels)
        ...
        ax1.set_xticks(x_labels)
        ax1.set_xlabel(
            r"$\mathrm{Observation~Cadence}$ $[\mathrm{yr}^{-1}]$", fontsize=labelsize
        )
        ax1.set_xticklabels([r"$%i$" % int(x) for x in x_labels], fontsize=axissize)
    elif var_x == "sigma":
        scale = 10 ** round(np.log10(xlabel_min))
        ...
        ax1.set_xticks(x_labels)
        ax1.set_xlabel(
            r"$\mathrm{Pulsar~Timing~RMS}$ $[\mathrm{ns}]$", fontsize=labelsize
        )
    ...
    if var_y == "M":
        ...
        ax1.set_yticklabels(
            [
                r"$10^{%i}$" % y if int(y) > 1 else r"$%i$" % (10 ** y)
                for y in np.log10(y_labels)
            ],
            fontsize=axissize,
        )
    elif var_y == "q":
        ...
        ax1.set_yticklabels(
            [y if int(y) < 1 else int(y) for y in y_labels], fontsize=axissize
        )
        ...
        ax1.set_yticklabels(
            [r"$10^{%.0f}$" % y for y in np.log10(y_labels)], fontsize=axissize
        )
        ax1.legend(loc="lower left")
    elif var_y == "A_IFO":
        ax1.axhline(
        ...
        y_labels = (
            np.arange(round(ylabel_min / scale), round(ylabel_max / scale) + 1, 1)
            * scale
        )
        ax1.set_yticks(y_labels)
        ...
        if yaxis_type == "log":
            ...
            ax1.set_yticklabels(
                [
                    r"$%.1f \times 10^{%.0f}$"
                    % (y / 10 ** int(np.log10(y)), np.log10(y))
                    if np.abs(int(np.log10(y))) > 1
                    else "{:g}".format(y)
                    for y in y_labels
                ],
                fontsize=axissize,
